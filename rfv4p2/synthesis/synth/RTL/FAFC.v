...
input [9+16-1:0] SPI_FCW_O;
input [7:0] SPI_FCW_MGN_O;
input [4:0] SPI_FAFC_FMULTI_O;
input [16-1:0] SPI_TDCRESNORM_O; // TDC_res_norm = Tres/(Tref)/M

output [9-1:0] FAFC_OTWP;
output [9-1:0] FAFC_OTWN;
...
wire [12+16:0] fcw_measure;
wire [12+16:0] SPI_FCW_O_ext;

wire [9+16-1:0] fcw_margin;
wire [9-1:0] FCW_I;
...
USWI1WF16SHIFT #(.width_int(16), .width_frac(6)) U0_TDCCODENORM_PRO ( .PRO(tdccnt_norm), .MULTIAI(SPI_TDCRESNORM_O), .MULTIBF(tdccnt) );
// assign diva = tdccnt_norm;
assign divb = (SPI_FAFC_FMULTI_O-1);
// USWI16DIV #(.WI(6+16)) U0_DIV ( .DIVA(diva), .DIVB(divb), .QUO(quo), .REM(rem) );
// assign tdccalc = (1'b1 << 16) + (tdccnt_sign? quo: (~quo+1'b1));
assign tdccalc = (1'b1 << 16) + (tdccnt_sign? tdccnt_norm: (~tdccnt_norm+1'b1));
...
always @ (posedge REF or negedge NRST1_pre) begin
	if (!NRST1_pre) begin
		...
		if (FREQLOCK2) begin
			...
			FAFC_CTRL <= ((poscnt==9-1)&(refcnt == refcnt_max-1))? 1'b0: 1'b1;
		end
	end
end
...
always @ (posedge REF or negedge NRST2) begin
	if (!NRST2) begin
		...
		if (FREQLOCK2) begin
			...
			FAFC_CTRL_in <= 1'b1;
			FREQLOCK2 <= ((poscnt==9-1)&(refcnt == refcnt_max-1))? 1'b1: 1'b0;
		end
	end
end
...
always @ (posedge REF or negedge NRST2) begin
	if (!NRST2) begin
		...
		if (refcnt == refcnt_max-1) begin 
			refcnt <= 0;
			poscnt <= (poscnt<9)? (poscnt + 1): poscnt;
			sar_win <= (poscnt>=9-1)? 1'b0: 1'b1;
		end else begin
			refcnt <= FAFC_CTRL_in? refcnt + 1: 0;
			sar_win <= (poscnt>=9)? 1'b0: 1'b1;
		end
	end
end
...
always @* begin
	if (sar_assert) begin
		if (poscnt<(9-1)) begin
			if (comp) sarcode = sarcode_dff - (1'b1<<(9-poscnt-2));
			else sarcode = sarcode_dff + (1'b1<<(9-poscnt-2));
		end else begin
...
always @ (posedge REF or negedge NRST) begin
	...
	else MMDNRST_reg <= ((poscnt==9-1)&(refcnt == refcnt_max-1))? 1'b0: 1'b1;
end

assign MMDNRST_FAFC = SPI_FAFC_EN? MMDNRST_reg: SPI_NARST;

// MMD rst initial
assign FCW_I = SPI_FCW_O[9+16-1:16];
